...
from mt_levy.components.actor import Actor
from mt_levy.components.critic import Critic
...
class SAC:
    ...
    def update(
        self, batch: tuple[NDArray, NDArray, float, NDArray, bool]
    ) -> dict[str, float]:
        obs, act, rwd, nobs, done = map(self._tensor, batch)  # type: ignore
    ...
    def _update_actor(
        self, obs: Tensor, _act: Tensor, _logp: Tensor, temp: Tensor
    ) -> Tensor:
