...
class SparseSAC:
    ...
    def __init__(
        ...
        class Sigmoid(nn.Module):
            ...
            def forward(self, x): return torch.sigmoid(2 * (x - 1))
    ...
    def update(self, batch: Tuple[np.ndarray, ...]) -> Dict[str, float]:
        ...
        # CQL
        # ood_q1 = self.critic1(torch.cat([obs, _act.detach()], dim=-1))
        # ood_q2 = self.critic2(torch.cat([obs, _act.detach()], dim=-1))
        # cql_loss = torch.mean(torch.minimum(ood_q1, ood_q2) - torch.minimum(q1, q2))
        # _critic_loss = critic_loss + 1e-3 * cql_loss
